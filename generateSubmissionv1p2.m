...
submissionFilePath = strcat(FilePath,'FlightQuestSimulator-Rev3\OneDaySimulatorFiles\submission1.csv');
%submissionFilePath = strcat(FilePath,'mysubmission.csv');
airportsFilePath = strcat(FilePath, 'airports.csv');
delimiter = ',';
startRow = 2;
stepDistanceMiles = 90;
...
for i = 1:1:totalFlights
    ...
    descentSlope = (FinalAltitudeFeet - AltitudeFeet)/totalDistanceKm;
    descentIntercept = FinalAltitudeFeet - descentSlope*totalDistanceKm;
    %% [fuelburn, TIME] = runFlight(climbAltitudeMeters, cruiseAltitudeMeters, totalDistance, descentTime)
    %[totalFuelBurn(i), flightTime(i)] = runFlight(AltitudeFeet*feetToMeters,AltitudeFeet*feetToMeters,totalDistanceKm*1000,150);
    
    % If destination is less than stepDistanceMiles away then pass landing
    % zone by 25 miles, 2000 ft, 0 knots as a midpoint between source and
    % destination.
    if totalDistanceKm < (stepDistanceMiles/kmToMile)
        ...
        stepCount = round(totalDistanceKm / (stepDistanceMiles/kmToMile));
        ...
        for j = 1:1:length(lats)
            if (j < length(lats))
                AltitudeFeet = descentSlope*i*(stepDistanceMiles/kmToMile)+descentIntercept;
